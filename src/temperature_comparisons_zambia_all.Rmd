...
tm <- tm %>% 
  filter(country == "Zambia" & station != "Moorings")

tm_chirts <- readRDS(here("data", "station", "cleaned", "temperature", "zambia_chirts.RDS"))

tm <- full_join(tm, tm_chirts, by = c("station", "date"))

tm$station <- factor(tm$station, levels = c("Kasama", "Mansa", "Mpika",
                                            "Magoye", "Choma", "Livingstone"))
...
```{r products}
d <- data.frame(Product = c("ENACTS", "ERA5", "ERA5-Land", "CHIRTS"),
                Resolution = c(0.05, 0.25, 0.1, 0.05),
                `Time Interval` = c("Daily", "Hourly", "Hourly", "Daily"),
                Coverage = c("Selected Countries", "Global", "Global", "Global"),
                Method = c("Combined Station + Reanalysis", "Reanalysis", "Reanalysis", "Combined Station + Satellite + Reanalysis"),
                check.names = FALSE)
...
```{r year_su35}
ggplot(year_ext_long %>% 
         filter(source != "era5land" & station %in% c("Magoye", "Choma", "Livingstone")), 
...
ext_stats_su35 %>% 
  filter(ind == "su35" & station %in% c("Magoye", "Choma", "Livingstone")) %>%
  pivot_wider(id_cols = c(stat, station), names_from = source, values_from = value) %>%
  arrange(stat, station) %>%
  kable(digits = 2) %>%
  skable()
