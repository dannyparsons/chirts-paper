...
husbands_temp <- daily_temp %>% filter(station == "Husbands")

daily_temp <- daily_temp %>% filter(station != "Husbands")

station_metadata <- readRDS(here("data", "station", "processed", "stations_metadata.RDS"))
temp_metadata <- station_metadata %>%
  filter(station %in% unique(daily_temp$station) & station != "Husbands")

zambia_metadata <- station_metadata %>%
  filter(country == "Zambia")

caribbean_metadata <- station_metadata %>%
  filter(station == "Husbands")
...
# ERA5 Import Caribbean ---------------------------------------------------

files <- list.files(here("data", "era5"), pattern = "husbands-2m_temperature", full.names = TRUE)

nc <- nc_open(files[1])
stopifnot(min(nc$dim$longitude$vals) < min(caribbean_metadata$longitude))
stopifnot(max(nc$dim$longitude$vals) > max(caribbean_metadata$longitude))
stopifnot(min(nc$dim$latitude$vals) < min(caribbean_metadata$latitude))
stopifnot(max(nc$dim$latitude$vals) > max(caribbean_metadata$latitude))
xs <- nc$dim$longitude$vals
ys <- nc$dim$latitude$vals
resx <- xs[2] - xs[1]
resy <- ys[2] - ys[1]
...
xy_points <- expand.grid(xs, ys)
xy_extract <- closest_point(points = xy_points, 
                            target = caribbean_metadata %>% dplyr::select(longitude, latitude))
xy_extract$station <- caribbean_metadata$station
xy_extract$req_longitude <- caribbean_metadata$longitude
xy_extract$req_latitude <- caribbean_metadata$latitude
xy_extract$dist <- apply(xy_extract, 1, function(r) {
  sp::spDistsN1(matrix(as.numeric(c(r[["Var1"]], r[["Var2"]])), ncol = 2),
                as.numeric(c(r[["req_longitude"]], r[["req_latitude"]])))
}
...
saveRDS(xy_extract, here("data", "station", "cleaned", "temperature", "temp_locations_caribbean.RDS"))
...
temp_era5_husbands_hour <- bind_rows(era5_dfs) %>%
  mutate(date_time = as.POSIXct(time * 60 * 60,
                                origin = as.POSIXct("1900/01/01 00:00:00"), tz = "GMT"),
         date = as.Date(date_time),
         # Temperature recording time ~ 6am
         # ERA5 record at 6am is instantaneous
         # So take 7am as the first reading
         # Formula: lead = start_time - UTC_diff
         # e.g. start_time = 7am, UTC_diff = -4, lead = 7 - (-4) = 11
         t2m_lead = dplyr::lead(t2m - 273.15, 11),
         hour = dplyr::lead(as.numeric(format(date_time, format = "%H")), 11)
  )
temp_era5_husbands <- temp_era5_husbands_hour %>%
  group_by(station, date) %>%
  summarise(tmin_era5 = min(t2m_lead), 
            tmax_era5 = max(t2m_lead))

temp_era5_husbands_tmintime <- temp_era5_husbands_hour %>%
  group_by(station, date) %>%
  filter(t2m_lead == min(t2m_lead)) %>%
  mutate(n = n()) %>%
  filter(n == 1) %>%
...
temp_era5_husbands_tmaxtime <- temp_era5_husbands_hour %>%
  group_by(station, date) %>%
  filter(t2m_lead == max(t2m_lead)) %>%
  mutate(n = n()) %>%
  filter(n == 1) %>%
...
temp_era5_husbands_time <- full_join(temp_era5_husbands_tmintime, temp_era5_husbands_tmaxtime, by = c("station", "date"))

temp_era5_husbands <- left_join(temp_era5_husbands, temp_era5_husbands_time, by = c("station", "date"))

temp_gridded_husbands <- left_join(husbands_temp, temp_era5_husbands, by = c("station", "date"))

# ggplot(temp_era5_husbands, aes(x = tmin_hour)) +
#   geom_bar()


# ENACTS Import Zambia ----------------------------------------------------


files <- list.files(here("data", "enacts", "zambia", "tmin"), full.names = TRUE)
names(files) <- list.files(here("data", "enacts", "zambia", "tmin"), full.names = FALSE)
nc <- nc_open(files[1])
...
for(j in seq_along(files)) {
  name <- substr(names(files)[j], 10, 17)
  for(i in seq_len(nrow(xy_extract))) {
...
zambia_enacts <- zambia_enacts %>% select(station, date, tmin_enacts = temp)

zambia <- left_join(zambia, zambia_enacts, by = c("station", "date"))

saveRDS(zambia_enacts, here("data", "station", "cleaned", "temperature", "zambia_enacts.RDS"))

# Merge all ---------------------------------------------------------------

temp_gridded <- bind_rows(temp_gridded, temp_gridded_husbands)

saveRDS(temp_gridded, here("data", "station", "cleaned", "temperature", "temp_gridded.RDS"))
